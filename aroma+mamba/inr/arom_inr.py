...
def main(cfg: DictConfig) -> None:
    ...
    RESULTS_DIR = cfg.wandb.save_path
    ...
    space_dim = trainset.space_dim
    # trainset images of shape (N, Dx, Dy, output_dim, T)

    # print("input_dim", input_dim)

    # transforms datasets shape into (N * T, Dx, Dy, C)
    trainset = rearrange_data(trainset, dataset_name)  # 因为是训练重建能力，所以输入只有一个时间步
    ...
    inr = (
        AROMAEncoderDecoderKL(
            ...
            out_dim=val_dim,
            hidden_dim=cfg.inr.hidden_dim,  # dimension after patchify
            mlp_ratios=cfg.inr.mlp_ratios,  # control hidden dim in mlp
            drop_rate=0.,
            drop_path_rate=0.,
            norm_layer=torch.nn.LayerNorm,
            patch_sizes=cfg.inr.patch_size,
            H=cfg.inr.H,  # input dimension after padding
            W=cfg.inr.W,
            num_mamba=cfg.inr.num_mamba,
            ...
            include_multi_pos=cfg.inr.include_multi_pos,
            depatch_each_step=cfg.inr.depatch_each_step,
            simple_decoder=cfg.inr.simple_decoder,
            use_mamba_decoder=cfg.inr.use_mamba_decoder,

            decoder_dim=cfg.inr.decoder_dim,
    ...
    for param_group in optimizer_inr.param_groups:
        param_group["lr"] = 1e-4
    print("lr: " + str(optimizer_inr.param_groups[0]['lr']))
    ...
    for step in range(epoch_start, epochs):
        ...
        for substep, (images, coords, idx) in enumerate(train_loader):
            ...
            images = images.reshape(n_samples, -1, val_dim)
            coords = coords.reshape(n_samples, -1, space_dim)
            if dataset_name != "shallow-water-dino":
                coords = coords * 2 - 1  # 从 [0, 1] 区间归一化到 [-1, 1] 区间
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                print(f"Epoch {epoch}: Loss is NaN or Inf, skipping step.")
                continue
            """

            if torch.isnan(loss) or torch.isinf(loss):
                raise ValueError(f"Epoch {step}: Loss is NaN or Inf, stopping training.")
            ...
            for name, param in inr.named_parameters():
                if param.grad is not None:
                    if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                        raise ValueError(f"NaN or Inf detected in gradient of {name}, stopping training.")
        ...
        if True in (step_show, step_show_last):
            ...
            print("Epoch: " + str(step) + " test_loss: " + str(test_loss) +
                  " kl_test: " + str(kl_test) +
                  " train_loss: " + str(train_loss) +
                  " kl_train: " + str(kl_train))
