...
def triton_cross_scan_flex(
    ...
    #get the block which is processing the code now, each block will process the function once
    i_h, i_w = (i_hw // NW), (i_hw % NW) #get 2D space index
    _mask_h = (i_h * BH + tl.arange(0, BH)) < DH
    #i_h * BH is the line number of data, i_h is the line number of block, i_h * BH + tl.arange(0, BH) is line number that current thread is processing
    ...
    neg_h = (DH - i_h * BH - 1 - tl.arange(0, BH)[:, None]) #invert order
    neg_w = (DW - i_w * BW - 1 - tl.arange(0, BW)[None, :])
    if scans == 0:
        # none; trans; flip; trans + flip;
        HWRoute0 = pos_h * DW + pos_w # must use the length of the whole block
    ...
    if onebyone == 0:
        x_ptr_base = x + i_b * _tmp1 + (i_c * BC * DH * DW if x_layout == 0 else i_c * BC) #the order is (NH * NW, NC, B), is reversed
        if x_layout == 0:
