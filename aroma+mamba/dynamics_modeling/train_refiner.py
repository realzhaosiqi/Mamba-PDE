...
class TokenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_polarized_mean:
            mask = torch.exp(0.5 * logvar) > self.threshold #mask掉std过大的维度，因为该维度不稳定
            mu = mu.clone()
...
def main(cfg: DictConfig) -> None:
    ...
    run.tags = ("mamba",) + (dataset_name,) + (f"sub={sub_tr}",)
    ...
    val_dim = 2 if cfg.data.dataset_name == "shallow-water-dino" else 1

    inr = (
        AROMAEncoderDecoderKL(
            space_dim=space_dim,  # coordiate dimension shape (B, L, C)
            val_dim=val_dim,  # value channel (B, L, C)
            out_dim=val_dim,
            hidden_dim=cfg_inr.inr.hidden_dim,  # dimension after patchify
            mlp_ratios=cfg_inr.inr.mlp_ratios,  # control hidden dim in mlp
            drop_rate=0.,
            drop_path_rate=0.,
            norm_layer=torch.nn.LayerNorm,
            patch_sizes=cfg_inr.inr.patch_size,
            H=cfg_inr.inr.H,  # input dimension after padding
            W=cfg_inr.inr.W,
            num_mamba=cfg_inr.inr.num_mamba,
            latent_dim=cfg_inr.inr.latent_dim,
            ...
            include_multi_pos=cfg_inr.inr.include_multi_pos,
            depatch_each_step=cfg_inr.inr.depatch_each_step,
            simple_decoder=cfg_inr.inr.simple_decoder,
            use_mamba_decoder=cfg_inr.inr.use_mamba_decoder,

            decoder_dim=cfg_inr.inr.decoder_dim,
    ...
    num_latents = (cfg_inr.inr.H//cfg_inr.inr.patch_size) * (cfg_inr.inr.W//cfg_inr.inr.patch_size)
    ...
    model = DiM(
        ...
        mlp_ratio=4.0,  # 4.0
        learn_sigma=False,
        H=cfg_inr.inr.H//cfg_inr.inr.patch_size,
        W=cfg_inr.inr.W//cfg_inr.inr.patch_size
    ).cuda()
    ...
    difference_weight = 1 / T_test if cfg.data.dataset_name == "mp-pde-burgers" else 1.0 #平均每一帧误差
    predict_difference = True if cfg.data.dataset_name == "mp-pde-burgers" else False #True 表示模型不是直接预测 x_t，而是预测 x_t - x_{t-1} 这种差值（类似 residual prediction），适用于动态系统建模
    use_statistics = cfg.model.use_statistics
    ...
    if resume_training:
        ...
        scheduler_lr.load_state_dict(checkpoint["scheduler_lr"])
    ...
    for step in range(epoch_start, epochs):
        ...
        for substep, batch in enumerate(train_loader):
            model.train()
            codes = batch["latent_codes"].cuda() #(B, N, C, T)
            ...
            for t in range(start, t_dim - 1):
                ...
                k = torch.randint(
                    ...
                    (x.shape[0],),
                    device=x.device,
                ) #在扩散模型中为每个样本选取一个随机时间步
                noise_factor = scheduler.alphas_cumprod.to(x.device)[k] #k 作为索引取出每个样本对应的保留因子
                noise_factor = noise_factor.view(-1, *[1 for _ in range(x.ndim - 1)])
                signal_factor = 1 - noise_factor #保留原始数据的程度
                ...
                pred = model(x, y_noised, k * time_multiplier) #模型是1000步时间尺度的时间步索引，time_multiplier将k映射到1000步上
                target = (noise_factor**0.5) * noise - (signal_factor**0.5) * y #  channel上合并还是直接在空间上合并
        ...
        for substep, batch in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                for t in range(t_dim - 1 - start):
                    y_noised = torch.randn_like(x).to(x)  # , dtype=x.dtype, device=x.device
                    ...
                    for k in scheduler.timesteps:
                        ...
                        pred = model(x, y_noised, time * time_multiplier)
            ...
            images = images.reshape(n_samples, -1, val_dim, t_dim)
            coords = coords.reshape(n_samples, -1, space_dim, t_dim)
            if cfg.data.dataset_name != "shallow-water-dino":
                coords = coords * 2 - 1
        ...
        print("Epoch: " + str(step) + " code_test_mse: " + str(code_test_mse) +
              " code_train_mse: " + str(code_train_mse) +
              #" pred_train_mse: " + str(pred_train_mse) +
              " pred_test_mse: " + str(pred_test_mse) +
              " mse_trajectory: " + str(mse) #+
              #" mse_in: " + str(mse_in) +
              #" mse_out: " + str(mse_out)
              )
        """
            ...
            print("Epoch: " + str(step) + " code_train_mse: " + str(code_train_mse))
        """
        if pred_test_mse < best_loss:
            best_loss = pred_test_mse
            if T_train != T_test:
                torch.save(
                    {
                        "cfg": cfg,
                ...
                print("model save Epoch: " + str(step) +
